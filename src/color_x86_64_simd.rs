...
#[inline]
fn egui_blend_u8_sse41(src: [u8; 4], dst: [u8; 4]) -> [u8; 4] {
    return unsafe { egui_blend_u8_sse41_impl(src, dst) };

    #[target_feature(enable = "sse4.1")]
    unsafe fn egui_blend_u8_sse41_impl(src: [u8; 4], dst: [u8; 4]) -> [u8; 4] {
        let alpha = src[3];
        if alpha == 255 {
            return src;
        }
...
/// blend fn is (ONE, ONE_MINUS_SRC_ALPHA)
#[inline]
pub fn egui_blend_u8_slice_one_src_sse41(src: [u8; 4], dst: &mut [[u8; 4]]) {
    unsafe { egui_blend_u8_slice_one_src_sse41_impl(src, dst) }

    #[target_feature(enable = "sse4.1")]
    unsafe fn egui_blend_u8_slice_one_src_sse41_impl(src: [u8; 4], dst: &mut [[u8; 4]]) {
        unsafe {
            let n = dst.len();
            if n == 0 {
                return;
            ...
            if a == 0 {
                ...
                while i + 1 < n {
                    ...
                    // Load two dst pixels
                    let d8 = _mm_cvtsi64_si128(read_unaligned(dst) as i64);
...
/// blend fn is (ONE, ONE_MINUS_SRC_ALPHA)
#[inline]
pub fn egui_blend_u8_slice_sse41(src: &[[u8; 4]], dst: &mut [[u8; 4]]) {
    unsafe { egui_blend_u8_slice_sse41_impl(src, dst) }

    #[target_feature(enable = "sse4.1")]
    unsafe fn egui_blend_u8_slice_sse41_impl(src: &[[u8; 4]], dst: &mut [[u8; 4]]) {
        unsafe {
            ...
            while i + 1 < n {
                ...
                let src8 = _mm_cvtsi64_si128(read_unaligned(src) as i64);
                // [0,0,0,0,rg,ba,rg,ba] -> [r,g,b,a,r,g,b,a]
...
/// blend fn is (ONE, ONE_MINUS_SRC_ALPHA)
#[inline]
pub fn egui_blend_u8_slice_tinted_sse41(src: &[[u8; 4]], tint: [u8; 4], dst: &mut [[u8; 4]]) {
    unsafe { egui_blend_u8_slice_tinted_sse41_impl(src, tint, dst) }

    #[target_feature(enable = "sse4.1")]
    unsafe fn egui_blend_u8_slice_tinted_sse41_impl(
        src: &[[u8; 4]],
        tint: [u8; 4],
        dst: &mut [[u8; 4]],
    ) {
        unsafe {
            ...
            while i + 1 < n {
                ...
                // Load two dst pixels
                let dst = dst.as_mut_ptr().add(i).cast::<u64>();
                let dst8 = _mm_cvtsi64_si128(read_unaligned(dst) as i64);
...
/// blend fn is (ONE, ONE_MINUS_SRC_ALPHA)
#[inline]
pub fn egui_blend_u8_slice_one_src_tinted_fn_sse41(
    src: [u8; 4],
    tint_fn: impl FnMut() -> [u8; 4],
    dst: &mut [[u8; 4]],
) {
    return unsafe { egui_blend_u8_slice_one_src_tinted_fn_sse41_impl(src, tint_fn, dst) };

    #[target_feature(enable = "sse4.1")]
    unsafe fn egui_blend_u8_slice_one_src_tinted_fn_sse41_impl(
        src: [u8; 4],
...
#[inline]
unsafe fn unorm_mult4x4_sse41(a: [u8; 4], b: [u8; 4]) -> [u8; 4] {
    return unsafe { unorm_mult4x4_sse41_impl(a, b) };

    #[target_feature(enable = "sse4.1")]
    unsafe fn unorm_mult4x4_sse41_impl(a: [u8; 4], b: [u8; 4]) -> [u8; 4] {
        let e1 = _mm_set1_epi16(0x0080);
